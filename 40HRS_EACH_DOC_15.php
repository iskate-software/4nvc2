...
// Opening and closing times
    ...
    $_POST['opene1m'] = substr($eslots[0],2,2) ;
    $_POST['closee1h'] = substr($eslots[1],0,2) ;
    $_POST['closee1m'] = substr($eslots[1],2,2) ;
    $_POST['opene2h'] = substr($eslots[2],0,2) ;
    $_POST['opene2m'] = substr($eslots[2],2,2) ;
    $_POST['closee2h'] = substr($eslots[3],0,2) ;
    $_POST['closee2m'] = substr($eslots[3],2,2) ;
    
    $_POST['opene3h'] = substr($eslots[4],0,2) ;
    $_POST['opene3m'] = substr($eslots[4],2,2) ;
    $_POST['closee3h'] = substr($eslots[5],0,2) ;
    $_POST['closee3m'] = substr($eslots[5],2,2) ;
    $_POST['opene4h'] = substr($eslots[6],0,2) ;
    $_POST['opene4m'] = substr($eslots[6],2,2) ;
    $_POST['closee4h'] = substr($eslots[7],0,2) ;
    $_POST['closee4m'] = substr($eslots[7],2,2) ;
    
    $_POST['opene5h'] = substr($eslots[8],0,2) ;
    $_POST['opene5m'] = substr($eslots[8],2,2) ;
    $_POST['closee5h'] = substr($eslots[9],0,2) ;
    $_POST['closee5m'] = substr($eslots[9],2,2) ;
    $_POST['opene6h'] = substr($eslots[10],0,2) ;
    $_POST['opene6m'] = substr($eslots[10],2,2) ;
    $_POST['closee6h'] = substr($eslots[11],0,2) ;
    $_POST['closee6m'] = substr($eslots[11],2,2) ;
...
// Now we deal with the no existing scheduling data this doctor, this day situation. Look in the CRITDATA file
         ...
         if (array_key_exists('DOCTOR',$row_dtls)) {
          // The doctor does not have a schedule yet, but is at least in the Doctor file. 
            $priority = $row_dtls['PRIORITY'] ; 
            
   //       $doctor = $row_dtls['DOCTOR'] ;  we already have this from the opening GETs
   
            $shortdoc = $row_dtls['SHORTDOC'] ;
            $initials = $row_dtls['DOCINIT'] ;
            $sequence = $priority ;
...
<td class="Labels" align="right"><select name="opene2h">
            ...
            <td class="Labels" align="left"><select name="closee2   m">
