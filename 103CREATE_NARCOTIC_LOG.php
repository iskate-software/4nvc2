...
//Check they have actually finished/saved with a non-zero amount.

 if ((isset($_POST['finish']) || isset($_POST['save'])) && $_POST['drawn'] != 0){

	$getnow = "SELECT NOW() as now, DATE_SUB(NOW(), INTERVAL 2 WEEK) AS window" ;
	$qg = mysqli_query($tryconnection, $getnow) or die(mysqli_error($mysqli_link)) ;
	$row_q = mysqli_fetch_assoc($qg) ;
	$get_when = "SELECT STR_TO_DATE('$_POST[treatdate]', '%m/%d/%Y') AS PROJ" ;
	$q_when = mysqli_query($tryconnection, $get_when) or die(mysqli_error($mysqli_link)) ;
	$row_when = mysqli_fetch_assoc($q_when) ;
...
// Then, check the quantities.
	 
if ($_POST['drawn'] > 0 &&  $_POST['drawn'] <  $_POST['used']) 
{
 echo '<script language="javascript">' ;
 echo 'alert("You cannot use more than you draw.")' ;
 echo '</script>' ;
 unset($_POST['save']) ;
 unset($_POST['finish']) ;
 $_POST['drawn'] = 0 ;
 $_POST['used'] = 0 ;
header("Location:GET_IT_RIGHT.html") ; 
}
else {
 ...
 $qtyrem = $available - $_POST['drawn'] ; // this is after everything is finished.

	$query_CHECKTABLE="SELECT CUSTNO FROM $treatmxx LIMIT 1";
...
// $now_get = "SELECT YEAR(NOW()) AS NOWYR, MONTH(NOW()) AS NOWMTH" ;
 ...
 if ($nmon1 < 0 ) {
   ...
   $set_up = "INSERT INTO NARCLOG (INVDTE,PETID,CUSTNO,WEIGHT,ITEM, B4, DRAWN, USED,QTYREM, ROA,TYPE,TIME, INVDOC,NYEAR, NMONTH,NWHODID,COMMENT)  
    ...
    $UPDATE_PURCH = "SELECT NPID,ITEM,DESCRIP,QTYREM,LASTDATE FROM NARCPUR WHERE ITEM = '$narc' AND QTYREM > 0 ORDER BY DATEPURCH,DATETIME,NPID " ;
    $query_upd = mysqli_query($tryconnection, $UPDATE_PURCH) or die(mysqli_error($mysqli_link)) ;
    $unfulfilled = $_POST['drawn'] ;  //i.e. y
    $done = 0 ;  // i.e. at this point, done to date.
//    $out = $unfulfilled ;  
   
    while ($row_purch = mysqli_fetch_assoc($query_upd) ) {
     if ($unfulfilled > 0 ) {
       $thisbot = $row_purch['QTYREM'] ;
       $rem = $thisbot - $unfulfilled ; 
       if ($rem < 0 ) { 
        $unfulfilled = $unfulfilled - $thisbot ; 
        $rem = 0 ;
       }
       else {
         $unfulfilled = 0 ; 
        }
       $WITHDRAW = "UPDATE NARCPUR SET QTYREM = '$rem', LASTDATE = STR_TO_DATE('$_POST[treatdate]', '%m/%d/%Y') WHERE NPID = '$row_purch[NPID]' LIMIT 1" ;
       $query_update = mysqli_query($tryconnection, $WITHDRAW) or die(mysqli_error($mysqli_link)) ;
     }
    
    }
// re-calc available in case they are doing multiple logs.   
$QTY_available = "SELECT SUM(QTYREM) AS QTYAVAIL FROM NARCPUR WHERE ITEM = '$narc' AND QTYREM > 0 " ;
$query_qty = mysqli_query($tryconnection, $QTY_available) or die(mysqli_error($mysqli_link)) ;
$row_qty = mysqli_fetch_assoc($query_qty) ;
$available = $row_qty['QTYAVAIL'] ;  // i.e. x
 ...
 $closewin = ("'../../INVENTORY/NARCOTIC_SHIPMENTS/NARCOTIC_SEARCH_SCREEN.php','_blank','width=800, height=600, toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no')") ; 
...
var out = document.add_n_log.drawn ;
var in = document.add_n_log.used ;
valid = true;
 ...
 if ((in > 0) && (in > out)) {
		alert ("Drawn quantity must be greater or equal to Used quantity. Please correct.");
		valid = false;
		self.close() ;
	}
return valid;
